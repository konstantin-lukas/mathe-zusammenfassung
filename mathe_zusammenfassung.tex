...
\section{Mengen}
	...
	\index{Mengen}
	\begin{multicols}{2}
		\begin{tcolorbox}[boxsep=0pt,top=.5cm,left=.5cm,right=.5cm, bottom=.5cm,arc=0pt,auto outer arc,colback=white,colframe=black, enlarge top by=.25cm]
	...
	\begin{multicols}{2}
		\begin{tcolorbox}[boxsep=0pt,top=.5cm,left=.5cm,right=.5cm, bottom=.5cm,arc=0pt,auto outer arc,colback=white,colframe=black]
			...
			Die reellen Zahlen $\mathbb{R}$ umfassen die rationalen Zahlen und die irrationalen Zahlen.
		\end{tcolorbox}
		\begin{tcolorbox}[boxsep=0pt,top=.5cm,left=.5cm,right=.5cm, bottom=.5cm,arc=0pt,auto outer arc,colback=white,colframe=black]
...
\pagebreak
	...
	\section{Vektorgeometrie}
		...
		\subsection{Rechnen mit Vektoren}
			...
			\subsection{Ebenen}
				...
				\begin{multicols}{2}
					\begin{flushleft}
						\textbf{Normalenform/Koordinatenform}\newline\newline
						Bei dieser Notationsform braucht man nur zwei Vektoren: Einmal die \textcolor{blue}{Stützvektor} und einmal den \textcolor{red}{Normalenvektor}. Der \textcolor{red}{Normalenvektor}\index{Normalenvektor} ist ein Vektor, der orthogonal\index{Orthogonal} zur \textcolor{teal}{Ebene} liegt. Daher kann dieser die beiden Spannvektoren ersetzen.
						\begin{tcolorbox}[boxsep=0pt,top=0cm,left=.5cm,right=.5cm, bottom=.5cm,arc=0pt,auto outer arc,colback=white,colframe=black]
							\begin{flalign*}
							&E:\vec{n}\cdot[\vec{x}-\overrightarrow{OP}]=0\text{ (Normalenform)}\\
	...
	\section{2D-Koordinatensystem}
		...
		\subsection{Kreisgleichungen}
			Bei Funktionen ist es so, dass jedem Abszess\index{Abszess} ($x$-Achsenwert) genau eine Ordinate\index{Ordinate} ($y$-Achsenwert) zugeordnet werden kann. Das ist eine Besonderheit von Funktionen und nicht des Koordinatensystems an sich. Im Koordinatensystem\index{Koordinatensystem} können wir noch viele andere Gebilde darstellen, so z.~B. auch einen Kreis\index{Kreis}.
